...
client.on("ready", () => {
  client.user.setActivity(`Alfa Uptime`);
  passport.use(
    new Strategy(
      {
...
linkss = linkA
 sahipp = sahip
  ...
  app.post("/add", checkAuth, (req, res) => {
    let ayar = req.body;
  let link = ayar["link"];
    if (!ayar["link"]) return res.send("Bağlantıyı Doldurmadınız!");

 if(db.get("linkler").map(z => z.url).includes(link)) {
      return res.send("Bu proje zaten uptime edilmekte!");
    } else {
      db.push("linkler", { url: link, owner: req.user.id });
      res.send("Link başarıyla sisteme kayıt edildi. Proje artık 7/24 aktif !");
    }
  });
